...
! Register diagnostic variables
      call self%register_horizontal_diagnostic_variable(self%id_chl,'chl','mg m-3','Ice algae in per cubic meter')
      call self%register_horizontal_diagnostic_variable(self%id_chlia,'chlia','mg m-2','Ice algae in per square meter')
      call self%register_horizontal_diagnostic_variable(self%id_fgrow,'fgrow','mmol-N m-3 d-1','ice algal growth rate')
      call self%register_horizontal_diagnostic_variable(self%id_fgraze,'fgraze','mmol-N m-3 d-1','ice algal grazing rate')
      call self%register_horizontal_diagnostic_variable(self%id_fmort,'fmort','mmol-N m-3 d-1','ice algal mortality rate')
      call self%register_horizontal_diagnostic_variable(self%id_fmort2,'fmort2','mmol-N m-3 d-1','ice algal quadratic mortality rate')
      call self%register_horizontal_diagnostic_variable(self%id_fmelt,'fmelt','mmol-N m-3 d-1','ice algal loss rate due to ice melting')
      call self%register_horizontal_diagnostic_variable(self%id_fpond,'fpond','mmol-N m-3 d-1','ice algal loss rate due to meltpond drainage')
      call self%register_horizontal_diagnostic_variable(self%id_fpondno3,'fpondno3','mmol-N m-3 d-1','no3 loss rate due to meltpond drainage')
      call self%register_horizontal_diagnostic_variable(self%id_fpondnh4,'fpondnh4','mmol-N m-3 d-1','nh4 loss rate due to meltpond drainage')
      call self%register_horizontal_diagnostic_variable(self%id_fpondsil,'fpondsil','mmol-N m-3 d-1','sil loss rate due to meltpond drainage')
      call self%register_horizontal_diagnostic_variable(self%id_fnit,'fnit','mmol-N m-3 d-1','nitrification in skeletal layer')
      call self%register_horizontal_diagnostic_variable(self%id_fno3up,'fno3up','mmol m-3 d-1','ice algal no3 uptake rate')
      call self%register_horizontal_diagnostic_variable(self%id_fsilup,'fsilup','mmol m-3 d-1','ice algal sil uptake rate')
      call self%register_horizontal_diagnostic_variable(self%id_fskelno3,'fskelno3','mmol m-3 d-1','no3 flux to skeletal layer')
      call self%register_horizontal_diagnostic_variable(self%id_fskelnh4,'fskelnh4','mmol m-3 d-1','nh4 flux to skeletal layer')
      call self%register_horizontal_diagnostic_variable(self%id_fskelsil,'fskelsil','mmol m-3 d-1','sil flux to skeletal layer')
      call self%register_horizontal_diagnostic_variable(self%id_ier,'ier','m d-1','Ice evolution rate (+ means growth, - means melting)')
      call self%register_horizontal_diagnostic_variable(self%id_lice,'lice','-','Limitation due to ice growth/melt')
      call self%register_horizontal_diagnostic_variable(self%id_llig,'llig','-','Limitation due to light')
      call self%register_horizontal_diagnostic_variable(self%id_lno3,'lno3','-','Limitation due to nitrate')
      call self%register_horizontal_diagnostic_variable(self%id_lsil,'lsil','-','Limitation due to silicate')
      call self%register_horizontal_diagnostic_variable(self%id_hnu,'hnu','m','Molecular sublayer thickness')
...
!--------------Ice Growth Limitation----------------
   lice = max(_ZERO_,1.0_rk-botmelt/self%crit_melt)
!----------------Light Limitation------------------
...
!--------------heat flux & assoc'd ice loss---------------
   ...
   fmelt = min(_ZERO_,ier*ia/self%zia)
...
!  fno3up   = grow*ia
   ...
   hnu      = self%nu/utaui
   fskelno3 = self%md_no3/hnu*(no3SW-no3)/self%zia+min(_ZERO_,ier*no3/self%zia)
   fskelnh4 = self%md_no3/hnu*(nh4SW-nh4)/self%zia+min(_ZERO_,ier*nh4/self%zia)
   fskelsil = self%md_sil/hnu*(silSW-sil)/self%zia+min(_ZERO_,ier*sil/self%zia)
   if (self%fmethod.eq.1) then ! ice-ocean flux based on melting/growth.
    fskelno3 = max(_ZERO_,ier*(no3SW-no3)/self%zia)+min(_ZERO_,ier*no3/self%zia)
    fskelnh4 = max(_ZERO_,ier*(nh4SW-nh4)/self%zia)+min(_ZERO_,ier*nh4/self%zia)
    fskelsil = max(_ZERO_,ier*(silSW-sil)/self%zia)+min(_ZERO_,ier*sil/self%zia)
    fmelt    = max(_ZERO_,ier*ph2/self%zia)+min(_ZERO_,ier*ia/self%zia)
   elseif (self%fmethod.eq.2) then ! combination of diffusion and melting/growth
    fskelno3 = fskelno3+max(_ZERO_,ier*(no3SW-no3)/self%zia)
    fskelnh4 = fskelnh4+max(_ZERO_,ier*(nh4SW-nh4)/self%zia)
    fskelsil = fskelsil+max(_ZERO_,ier*(silSW-sil)/self%zia)
    fmelt    = max(_ZERO_,ier*ph2/self%zia)+min(_ZERO_,ier*ia/self%zia)
   endif
