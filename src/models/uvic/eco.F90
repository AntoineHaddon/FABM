...
module uvic_eco
   ...
   !might need more use statements? 
...
!fabm.nml 
   ...
   call self%get_parameter(f_seed, 'f_seed','-', 'fraction of ice algal fux as ph2 seeding', default=0.0_rk)
   ...
   call self%get_parameter(w1 , 'w1 ','m/d', 'de1 sinking rate',default=6.0_rk)
   call self%get_parameter(w2 , 'w2 ','m/d', 'de2 sinking rate', default=6.0_rk)
   call self%get_parameter(mu1, 'mu1','1/d', 'ph1 maximum growth rate', default=2.0_rk)
   call self%get_parameter(mu2 , 'mu2 ','1/d', 'ph2 maximum growth rate', default=2.0_rk)
   call self%get_parameter(kn, 'kn','umol/L', ' no3 & nh4 half saturation constant', default=0.1_rk)
   !call self%get_parameter(al1, 'al1','m2/W/time', 'initial slope of P-I curve ([time] is same unit as pmax)', default=0)
   !call self%get_parameter(al2 , 'al2 ','m2/W/time', 'initial slope of P-I curve ([time] is same unit as pmax)', default=0)
   call self%get_parameter(rpp1, 'rpp1','1/time', 'maximum photosynthetic rate ([time] is same unit as alpha)', default=0.05_rk)
   call self%get_parameter(rpp2 , 'rpp2 ','1/time', 'maximum photosynthetic rate ([time] is same unit as alpha)', default=0.05_rk)
   call self%get_parameter(mp1, 'mp1','1/d', 'ph1 excretion rate ', default=0.05_rk)
   call self%get_parameter(mp2 , 'mp2 ','1/d', 'ph2 excretion rate', default=0.05_rk)
   call self%get_parameter(mz1, 'mz1','1/d', 'zo1 excretion rate ', default=0.1_rk)
   call self%get_parameter(mz2, 'mz2','1/d', 'zo2 excretion rate ', default=0.3_rk)
   call self%get_parameter(gz1, 'gz1','1/d', 'zo1 maximum grazing rate ', default=1.3_rk)
   call self%get_parameter(gz2, 'gz2','1/d', 'zo2 maximum grazing rate ', default=0.8_rk)
   call self%get_parameter(kz1, 'kz1','umol/L', 'zo1 grazing half saturation constant ',default=0.6_rk)
   call self%get_parameter(kz2, 'kz2','umol/L', 'zo2 grazing half saturation constant ', default=0.75_rk)
   call self%get_parameter(az1, 'az1','-', 'zo1 assimilation fraction ',default=0.7_rk)
   call self%get_parameter(az2, 'az2','-', 'zo2 assimilation fraction ',default=0.1_rk)
   call self%get_parameter(rc, 'rc','1/d', 'closure mortality rate', default=0.003_rk)
  ! call self%get_parameter(htlnh4, 'htlnh4','-', 'closure loss fraction to nh4', default=0)
  ! call self%get_parameter(htldet, 'htldet','-', 'closure loss fraction to det', default=0)
   call self%get_parameter(pp1, 'pp1','-', 'ph1 fraction as food for zo1', default=1.0_rk)
   call self%get_parameter(pp2, 'pp2','-', 'ph2 fraction as food for zo2', default=1.0_rk)
   call self%get_parameter(pd1, 'pd1','-', 'de1 fraction as food for zo1', default=0.5_rk)
   call self%get_parameter(pd2, 'pd2','-', 'de2 fraction as food for zo2', default=0.5_rk)
   call self%get_parameter(pz1, 'pz1','-', 'zo1 fraction as food for zo2', default=1.0_rk)
   call self%get_parameter(rd1, 'rd1','1/d', ' de1 remineralization rate', default=0.1_rk)
   call self%get_parameter(rd2, 'rd2','1/d', 'de2 remineralization rate', default=0.1_rk)
   call self%get_parameter(rd3, 'rd3','1/d', 'bsi remineralization rate', default=0.1_rk)
   call self%get_parameter(rpf, 'rpf','-', 'scale factor for nitrogen preference function', default=0.2_rk )
   call self%get_parameter(rn0, 'rn0','1/d', 'nitrification rate at 0 deg.C', default=0.03_rk)
   call self%get_parameter(knt, 'knt','1/deg.C', 'nitrification temperature coefficient', default=0.0693_rk)
   call self%get_parameter(qp, 'qp','-', 'ph1 & ph2 Q10 factor', default=2.0_rk)
   call self%get_parameter(qz, 'qz','-', 'zo1 & zo2 Q10 factor', default=3.0_rk )
   call self%get_parameter(qb, 'qb','-', 'bacteria Q10 factor', default=3.0_rk)
   call self%get_parameter(agg, 'agg', '1/d', 'ph2 aggregation rate',default=0.07_rk)
   call self%get_parameter(rsin, 'rsin','mol-Si/mol-N', 'ph2 Si:N ratio', default=2.0_rk)
   call self%get_parameter(ks, 'ks','umol/L', 'si half saturation constant', default=2.0_rk)
   call self%get_parameter(pmin, 'pmin','umol-N/L', 'background plankton concentration', default=0.01_rk)
...
! Register diagnostic variables
      ...
      call self%register_horizontal_diagnostic_variable(self%id_stemp,'stemp','deg.C','Temperature in the top layer')
...
!   _SET_SURFACE_EXCHANGE_(self%id_de2,-fialde2+(1-self%f_seed)*fmort2)
    _SET_SURFACE_EXCHANGE_(self%id_de2,-fialde2+0.7*fmort+fmort2)
