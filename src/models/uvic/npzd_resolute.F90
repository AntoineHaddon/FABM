...
!there isnt actually a yaml to read from? jpnote 
   ...
   call self%get_parameter(wp2,'wp2','m d-1','sinking speed for ph2',default=1.2_rk)
   call self%get_parameter(wd,'wd','m d-1','sinking speed for det',default=6.0_rk)
...
!read in no3_0 from eco ;;
  ...
  ! call self%get_parameter(self%rpi, 'rpi','', 'ratio of photoinhibition parameters (beta and pbm)', default=0.0_rk)
   ...
   call self%register_state_variable(self%id_ph2,'ph2','umol/L','Large phytoplankton (Diatoms)',initial_value=ph2_0,minimum=0.0_rk,vertical_movement=self%wp2)
   ...
   call self%register_state_variable(self%id_det,'det','umol/L','Detritus',initial_value=det_0,minimum=0.0_rk,vertical_movement=self%wd)                         
